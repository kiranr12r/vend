  ...
  async function handleStatusChange(vendorId: string, status: string) {
    ...
    try {
      const res = await fetch(`/api/vendors/${vendorId}/status`, {
        ...
        body: JSON.stringify({ status, changedBy: "admin@vendorlink.com" }),
      });
      if (res.ok) {
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="bg-white border-b border-gray-100 px-5 h-11 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <Link href="/vendors/new" className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 text-white rounded-lg text-xs font-semibold hover:bg-blue-700 transition-all">
        ...
        <div className="max-w-6xl mx-auto space-y-4">
          ...
          <div className="bg-white border border-gray-100 rounded-xl p-3 flex items-center gap-3 shadow-sm">
            <div className="flex-1 relative">
              ...
              <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by vendor name, GST or contact..." className="w-full pl-8 pr-3 py-2 border border-gray-200 rounded-lg text-xs focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-50 transition-all" />
            </div>
            <div className="flex items-center gap-1.5">
              {["ALL", "PENDING_APPROVAL", "APPROVED", "REJECTED", "DRAFT"].map((s) => (
                <button key={s} onClick={() => setStatusFilter(s)} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all duration-150 ${statusFilter === s ? "bg-blue-600 text-white" : "bg-gray-50 text-gray-500 hover:bg-gray-100"}`}>
          ...
          <div className="bg-white border border-gray-100 rounded-xl overflow-hidden shadow-sm">
            <div className="grid grid-cols-12 gap-3 px-4 py-2.5 bg-gray-50 border-b border-gray-100">
              {[
                ...
                { label: "Service", span: "col-span-2" },
                { label: "Contact", span: "col-span-2" },
                { label: "Bank A/C", span: "col-span-1" },
            ...
            {!loading && filtered.map((vendor, index) => (
              <div key={vendor.id} onClick={() => setSelected(vendor)} className="grid grid-cols-12 gap-3 px-4 py-3 border-b border-gray-50 last:border-b-0 hover:bg-blue-50/30 transition-all duration-150 cursor-pointer" style={{ animation: `slideUp 0.${Math.min(index + 1, 9)}s ease` }}>
                ...
                <div className="col-span-2 flex items-center">
                  <span className="text-xs bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-full font-medium truncate">{vendor.natureOfService || "—"}</span>
                </div>
                ...
                <div className="col-span-1 flex items-center">
                  <span className={`text-xs font-semibold ${vendor.bankAccounts?.length > 0 ? "text-emerald-600" : "text-gray-300"}`}>{vendor.bankAccounts?.length || 0}</span>
                </div>
                <div className="col-span-1 flex items-center">
                  <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${STATUS_STYLES[vendor.status] || "bg-gray-100 text-gray-500"}`}>{STATUS_LABELS[vendor.status] || vendor.status}</span>
                </div>
                <div className="col-span-1 flex items-center">
                  <button onClick={(e) => { e.stopPropagation(); setSelected(vendor); }} className="p-1.5 hover:bg-blue-100 rounded-lg transition-all">
          ...
          {!loading && (
            <p className="text-xs text-gray-400 text-center">Showing {filtered.length} of {vendors.length} vendors</p>
          )}
        </div>
      </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {selected.registeredMsme && <span className="text-xs bg-emerald-50 text-emerald-600 border border-emerald-100 px-2 py-0.5 rounded-full font-medium">MSME Registered</span>}
                  {selected.eInvoiceRequired && <span className="text-xs bg-blue-50 text-blue-600 border border-blue-100 px-2 py-0.5 rounded-full font-medium">E-Invoice Required</span>}
                  {!selected.registeredMsme && !selected.eInvoiceRequired && <span className="text-xs text-gray-300">No flags</span>}
                </div>
              </div>

              <div>
                <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Bank Accounts ({selected.bankAccounts?.length || 0})</p>
                {selected.bankAccounts?.length === 0 ? (
                  ...
                  selected.bankAccounts?.map((acc: any, i: number) => (
                    <div key={i} className="p-3 bg-gray-50 rounded-lg border border-gray-100 text-xs mb-2">
